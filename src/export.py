...
def real_quantize_fixed_symmetric(
    W: torch.Tensor,
    scales: torch.Tensor,
    group_size: int = 128,
    q_bits: int = 4,
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Quantize using stored fixed per-row/per-group symmetric scales."""
    q_max = 2 ** (q_bits - 1) - 1
    W = W.float().cpu()
...
def real_quantize_fixed_asymmetric(
    W: torch.Tensor,
    scales: torch.Tensor,
    zero_points: torch.Tensor,
    group_size: int = 128,
    ...
    """Quantize using stored fixed affine per-row/per-group qparams."""
    q_max = 2 ** q_bits - 1
    W = W.float().cpu()
    scales = scales.float().cpu()
    zero_points = zero_points.float().cpu()
    out_f, in_f = W.shape
    W_grouped, _, _ = _pad_and_group(W, group_size)
    W_int = _groupwise_quantize_asymmetric(
        W_grouped, scales, zero_points, q_max, in_f
    )
    return W_int, scales, zero_points
...
def save_sqat_permute_meta(meta: dict, output_dir: str) -> None:
    """Save sqat_permute metadata (boundary_perms, segment_perms, etc.) alongside model."""
    import os
...
def merge_and_export(
    ...
    # --- Save sqat_permute boundary metadata alongside the exported model ---
    if qat_mode == "sqat_permute" and sqat_permute_meta:
        save_sqat_permute_meta(sqat_permute_meta, output_dir)
