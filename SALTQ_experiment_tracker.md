...
| QLoRA fp16 上限(同 recipe,lr 2e-4,span) | 16 | — | **58.07**(MATH 10.64) | — | **5.09** @2048(5.35 @1024)✅ 16072408 |
...
> **第一臂已回(2026-09-03,job 16072408)**:QLoRA fp16 上界 = **5.0871** @2048 / 5.3503 @1024。
> 对照未微调的原始 fp16 base 5.4721 —— 微调确实起作用了(−0.385),而且落在 LoftQ/ApiQ 那批
> 5.2–7.8 的区间内,跨论文可比性成立。
>
> 同一个 job 还顺带产出 `-2bit-none-dequant-eval`(NF4 基座 + 导出量化器里的 INT2 RTN,**没有
> GPTQ**),PPL = **207,016**。**它不是本列的一行**(本列的 floor 是 merged→GPTQ,job 16072456),
> 但它是"PPL 比多选准确率灵敏得多"的直接证据:同一个模型在 CS 表里只会掉到接近随机猜(~36),
> 在 PPL 上却炸掉四个数量级。
