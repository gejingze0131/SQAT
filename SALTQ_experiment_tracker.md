...
>
> **†QEFT 那一行必须连它的裸基座一起读**:QEFT **未微调**的混合精度基座就已经是 **7.538**,
> 比两个纯 INT2 臂**微调之后**的成绩(8.817 / 10.154)还好。也就是说它在本列的领先主要来自
> **多出来的 0.75 个 bit**,微调只贡献了 7.538 → 6.683 = 0.86。job 16077188 在补测同一校准下
> 的**纯 INT2 g32 裸基座**,拿到之后就能把这 0.75 bit 直接标价。
>
> 已经能读出来的两件事:
...
> | QEFT | 348M | **1.580**(拟合最差) | ⏳ 16077188 | ⏳ | 6.683 | ⏳ |
> | QLoRA fp16 上界 | LoRA r=64 | — | 3.921 | 5.186 | 5.087 | **1.30×** |
> | GPTQ floor(量化后未训) | 0 | — | 9.307 | 11.786 | 10.273 | 1.10× |
>
> **训练拟合与测试泛化在四个受训臂之间完全反相关,没有例外。** 按末段 train loss 排序
> SALT-Q 1.237 < QA-LoRA 1.445 < QEFT 1.580,按 test PPL 排序**恰好倒过来**
> 10.154 > 8.817 > 6.683。SALT-Q 的 in-sample PPL 是 **1.50**(接近背下来),却是受训臂里
> 测试最差的;fp16 上界拟合训练集**最差**(3.92),泛化**最好**(5.09)。test/train 比值随可训
> 自由度单调上升:1.30 → 3.93 → 6.75。这是教科书式的过拟合阶梯。
>
> (QEFT 的 test 最好,同时训练拟合最差 —— 两个原因叠在一起:lr 最小(9.35e-5),以及它的
> 基座本来就多 0.75 bit。见上面 † 那一段。)
>
