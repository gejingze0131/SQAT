...
| QEFT(fp16 弱列,无 zp;weak lr 5e-5) | 2 | 32 | **48.90**(裸基座 0.00;补评 MATH **7.44**,复验 gsm8k 48.52,job 16046801) | — | ⏳ 16072464 |

> **Wiki2 列进度(2026-09-03):3/5 已回,边界已合拢。**
>
> | 臂 | 部署形态 | @2048 | @1024 | job |
> |---|---|---|---|---|
> | QLoRA fp16 **上界** | adapter 挂载态(NF4 反量化 + adapter 合并,未重量化) | **5.087** | 5.350 | ✅ 16072408 |
> | QA-LoRA | 合并后的 INT2 g32 | **8.817** | 9.446 | ✅ 16072461 |
> | QLoRA→GPTQ **floor** | merged fp16 → GPTQ INT2 g32(同 0.5M in-domain 校准) | **10.273** | 11.013 | ✅ 16072456 |
> | SALT-Q | 导出的统一低比特 ckpt | — | — | ⏳ 16072459 |
> | QEFT† | INT2 codes + 256 fp16 弱列(2.75 等效 bit) | — | — | ⏳ 16072464 |
>
> 已经能读出来的两件事:
...
>
> | 臂 | 可训参数 | train(166 窗) | validation | test | **test/train** |
> |---|---|---|---|---|---|
> | SALT-Q | 527M | **1.504** | 10.258 | 10.154 | **6.75×** |
> | QA-LoRA | ~160M | 2.243 | 8.860 | 8.817 | 3.93× |
> | QLoRA fp16 上界 | LoRA r=64 | 3.921 | 5.186 | 5.087 | **1.30×** |
> | GPTQ floor(量化后未训) | 0 | 9.307 | 11.786 | 10.273 | 1.10× |
>
> **训练 PPL 与测试 PPL 在受训臂之间完全反相关。** SALT-Q 的训练 PPL 是 **1.50**(接近背下来),
> 却是受训臂里测试最差的;fp16 上界拟合训练集**最差**(3.92),泛化**最好**(5.09)。
> test/train 比值随可训自由度单调上升:1.30 → 3.93 → 6.75。这是教科书式的过拟合阶梯。
>
...
>
> **结论**:按现在这个 recipe,Wiki2 列是**容量/正则化的排序**,不是量化质量的排序。要让它成为
> 一个公平的量化对照,需要一个**容量匹配或轮数更少的点**(全部臂一起改,否则不是单变量)。
> 候选:1 epoch(T=176,lr 再按 √(1845/176)=3.24 搬运),SALT-Q 的两个基座可以用
> `--permuted_base_dir` / `--saltq_base_dir` 复用,不必重建。**未跑,待定。**
>
