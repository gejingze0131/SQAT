...
| QWHA | 2 | 32 | — | — | — |
| **SALT-Q** | 2 | 32 | **40.94** | — | — |
| **SALT-Q**(span bcal gptq_latent,CS 经验 lr,zp×2,balanced 1k 校准) | 2 | 32 | **56.33**(旧格 40.94;补评 MATH **13.68**,全格最高、超 fp16 参考 10.64;复验 gsm8k 56.48,job 16045646) | — | 10.154 @2048(10.890 @1024)⚠️ 过拟合,见下 · 16072459 |
| QLoRA fp16 上限(同 recipe,lr 2e-4,span) | 16 | — | **58.07**(MATH 10.64) | — | **5.087** @2048(5.350 @1024)✅ 16072408 |
| QLoRA→GPTQ floor(58.07 merged ckpt 经 balanced 1k 校准 GPTQ INT2 g32) | 2 | 32 | **22.06**(MATH 2.16;健康生成,1319 distinct,非坏文件) | — | **10.273** @2048(11.013 @1024)✅ 16072456 |
| QA-LoRA(同 recipe,lr 5e-3,balanced 1k 基座) | 2 | 32 | **52.54**(MATH 11.04;train_loss 0.2646) | — | **8.817** @2048(9.446 @1024)✅ 16072461 |
| QEFT(fp16 弱列,无 zp;weak lr 5e-5) | 2 | 32 | **48.90**(裸基座 0.00;补评 MATH **7.44**,复验 gsm8k 48.52,job 16046801) | — | **6.683** @2048(7.081 @1024);裸基座 **7.538** ⚠️ 见下 · 16076231 |

> **Wiki2 列进度(2026-09-03):3/5 已回,边界已合拢。**
>
> | 臂 | 部署形态 | 等效 bit | @2048 | @1024 |
> |---|---|---|---|---|
> | QLoRA fp16 **上界** | adapter 挂载态(NF4 反量化 + adapter 合并,未重量化) | 16 | **5.087** | 5.350 |
> | QEFT† | INT2 codes + 256 fp16 弱列 | **2.75** | **6.683** | 7.081 |
> | QA-LoRA | 合并后的 INT2 g32 | 2.00 | **8.817** | 9.446 |
> | SALT-Q | 导出的统一低比特 ckpt(identity merge) | 2.00 | **10.154** | 10.890 |
> | QLoRA→GPTQ **floor** | merged fp16 → GPTQ INT2 g32(同 0.5M in-domain 校准) | 2.00 | **10.273** | 11.013 |
>
> ### 未微调锚点(job 16077188)—— 没有这三行,上表读不对
>
> | 未微调基座 | 等效 bit | test @2048 | validation |
> |---|---|---|---|
> | fp16 Llama-2-7B(协议锚点) | 16 | **5.472** | — |
> | QEFT 基座(k=256 列留 fp16) | **2.75** | **7.538** | — |
> | **纯 INT2 g32 GPTQ**(同 0.5M in-domain 校准,预训练 Llama 直接量化) | 2.00 | **13.438** | 13.736 |
>
> **0.75 个 bit 值 5.90 PPL**(13.438 → 7.538),吃掉了 2bit 全部代价(5.472 → 13.438 = +7.97)
> 的 **74%**。这个数比任何一个臂的微调收益都大得多。于是本列的正确读法是**从各自的起点算增量**:
>
> | 臂 | 等效 bit | 起点(未微调) | 微调后 | **微调收益** |
> |---|---|---|---|---|
> | QEFT | 2.75 | 7.538 | 6.683 | **−0.855** |
> | QA-LoRA | 2.00 | 13.438 | 8.817 | **−4.621** |
> | SALT-Q | 2.00 | 13.438 | 10.154 | **−3.284** |
> | QLoRA→GPTQ floor | 2.00 | 13.438 → (先 fp16 微调再 PTQ) | 10.273 | −3.165 |
>
> **所以 QEFT 的 6.683 不是微调赢的,是比特预算赢的。** 它从一个好得多的起点出发,微调只走了
> 0.86;两个纯 INT2 臂从 13.438 出发,QA-LoRA 走了 4.62、SALT-Q 走了 3.28。论文里这一行必须
> 按 §T1 已有的口径标成 "2 + fp16 / 2.75 等效 bit",并且**把裸基座 7.538 一起列出来**,否则
> 读者会把比特预算读成方法优势。
>
> **同时这也让 SALT-Q 的问题更清楚**:它 10.154,而"先 fp16 微调再 PTQ"的 floor 是 10.273 ——
> 只赢 **0.12**。QA-LoRA 在同一起点上赢了 1.46。SALT-Q 的容量优势被过拟合吃光了。
>
> 已经能读出来的两件事:
> * **上界/floor 的比值是 2.02×**(5.087 → 10.273)。CS 表里同样这对是 77.75 → 66.22 = 15% 的
>   相对差;PPL 上是 **102%**。这就是"PPL 是连续指标、比多选准确率灵敏得多"的量化版本 ——
>   方法之间的空间在这一列宽得多。
> * **QA-LoRA 8.817 已经打过 floor 1.46 PPL**,即 QAT-in-the-loop 在本列确实有效,不是噪声。
>
> ### ⚠️ 但这一列目前排的是**正则化**,不是量化质量 —— 别直接写进论文
>
> SALT-Q 落在 **10.154**,在 QA-LoRA 之上、几乎贴着 floor,与其它每一格的排序相反。
> 诊断 job **16076252** 把每个臂的 in-sample 训练集 PPL(取前 166 窗,与测试集等代价)和
> 第二个 held-out 的 validation split 都测了:
>
> | 臂 | 可训参数 | 末段 train loss | train PPL(166 窗) | validation | test | **test/train** |
> |---|---|---|---|---|---|---|
> | SALT-Q | 527M | **1.237**(拟合最好) | **1.504** | 10.258 | 10.154 | **6.75×** |
> | QA-LoRA | ~160M | 1.445 | 2.243 | 8.860 | 8.817 | 3.93× |
> | QEFT | 348M | **1.580**(拟合最差) | 3.516 | 6.739 | 6.683 | 1.90× |
> | QLoRA fp16 上界 | LoRA r=64 | — | 3.921 | 5.186 | 5.087 | **1.30×** |
> | GPTQ floor(量化后未训) | 0 | — | 9.307 | 11.786 | 10.273 | 1.10× |
> | 纯 INT2 裸基座(未训) | 0 | — | — | 13.736 | 13.438 | — |
>
> **训练拟合与测试泛化在四个受训臂之间完全反相关,一个例外都没有,而且是单调的:**
>
> ```
> train PPL   1.504  <  2.243  <  3.516  <  3.921        (拟合从好到差)
> test  PPL  10.154  >  8.817  >  6.683  >  5.087        (泛化恰好倒过来)
> test/train  6.75×  >  3.93×  >  1.90×  >  1.30×        (随可训自由度单调)
>             SALT-Q   QA-LoRA   QEFT     QLoRA fp16
> ```
>
> SALT-Q 的 in-sample PPL 是 **1.50**(接近背下来),却是受训臂里测试最差的;fp16 上界拟合训练集
> **最差**(3.92),泛化**最好**(5.09)。这是教科书式的过拟合阶梯。
>
> (QEFT 拟合最差 / 泛化最好有两个叠加的原因:lr 最小(9.35e-5),以及基座多 0.75 bit —— 见上面
> 的未微调锚点小节,那 0.75 bit 单独就值 5.90 PPL。)
>
> 原因是语料规模:WikiText-2 只有 **2.87M token**,比 CS-170k 小 12×、比 MetaMath 小 50×,
> 而 recipe 跑了 3 遍。§lr 定律搬运的是**位移**(优化器动力学),它不搬运泛化 —— 在 12× 小的
> 数据上让参数走同样远,恰好就是过拟合的配方。
>
> **两个已排除的解释**(不是它们):
> * **不是 merge/export 损伤**:SALT-Q 的 `deploy == train` 在每个检查层上 `max|Δ| = 0`。
> * **不是基座坏了**:冻结码重建误差 median 55.8% / worst 165.5%,夹在 CS 格的 54.7/159.6 与
>   MetaMath 格的 58.3/166.0 之间,正常。(自动分段确实给出 `[1, 31]` 而不是 MetaMath 的
>   `[2, 30]`,Seg1 能量覆盖 23.4% vs 35.0% —— 记录在案,但它解释不了训练 PPL 1.50。)
> * validation 与 test 在每个臂上都吻合(10.26/10.15、8.86/8.82、5.19/5.09),所以 test 的
>   排序是真的,不是单个 split 的偶然。
>
> **结论**:按现在这个 recipe,Wiki2 列是**容量/正则化的排序**,不是量化质量的排序。
>
> ### ✅ 1-epoch 对照格已出分(job 16077163;floor 16077164 ⏳;诊断 16078835 ⏳)
>
> | 臂 | 等效 bit | 3ep @2048 | @1024 | **1ep @2048** | @1024 | Δ | 3-ep 的 test/train |
> |---|---|---|---|---|---|---|---|
> | QLoRA fp16 上界 | 16 | 5.087 | 5.350 | **4.845** | 5.076 | −0.24 | 1.30× |
> | QEFT† | 2.75 | 6.683 | 7.081 | **6.024** | 6.354 | −0.66 | 1.90× |
> | QA-LoRA | 2.00 | 8.817 | 9.446 | **6.296** | 6.652 | **−2.52** | 3.93× |
> | SALT-Q | 2.00 | 10.154 | 10.890 | **7.002** | 7.404 | **−3.15** | 6.75× |
> | QLoRA→GPTQ floor | 2.00 | 10.273 | 11.013 | **8.873** | 8.975 | −1.40 | 1.10× |
>
> **建议主表用 1-epoch 格**(3-epoch 格降为容量/正则化消融)。理由见下面第 4 点。
>
> **1. 过拟合诊断被直接证实。** 每个臂都变好了,而且**改善量与它 3-epoch 时的 test/train 比值
> 严格同序**:6.75×→−3.15、3.93×→−2.52、1.90×→−0.66、1.30×→−0.24。过拟合越重的臂,少跑两遍
> 数据赚得越多。这不是事后解释,是先诊断、后预测、再验证。
>
> **2. 但 2.00-bit 的排序没有翻过来。** QA-LoRA 6.296 仍在 SALT-Q 7.002 之前,差距从 1.34
> 收窄到 **0.71**(约一半)。所以过拟合解释了这个 gap 的**一半**,不是全部 —— 剩下的 0.71 需要
> 另一个解释。这与配置头里预先写下的 caveat 一致:lr 按 √3 搬运后**位移不变**,参数走得还是
> 一样远,只是过数据的遍数少了。**下一个杠杆是位移本身(不搬运 lr),那是另一个格。**
>
> **3'. 1-epoch 格的过拟合诊断(job 16078835,同一协议、同样 166 个训练窗)**
>
> | 臂 | train PPL | validation | test | **test/train** | (3-ep 的比值) |
> |---|---|---|---|---|---|
> | SALT-Q | **4.166**(仍是拟合最好的) | 7.059 | 7.002 | **1.68×** | 6.75× |
> | QA-LoRA | 4.838 | 6.358 | 6.296 | 1.30× | 3.93× |
> | QEFT | 4.807 | 6.079 | 6.024 | 1.25× | 1.90× |
> | QLoRA fp16 上界 | 4.631 | 4.935 | 4.845 | **1.05×** | 1.30× |
> | GPTQ floor | 8.401 | 8.665 | 8.873 | 1.06× | 1.10× |
>
> **过拟合塌下去了,但没有消失,而且残余全在 SALT-Q 上**:所有比值从 1.05–6.75 收进 1.05–1.68,
> 但 SALT-Q 依然是 **in-sample 拟合最好(4.166,受训臂里最低)、held-out 最差(7.002)**,
> 比值 1.68 仍明显高于其余三臂的 1.05–1.30。所以第 2 点里那个残余的 0.71 就是**没被 1 epoch
> 拿掉的那部分过拟合** —— 与 caveat 一致:位移没变,527M 可训参数走得还是一样远。
>
> **4. 这一格里 QAT-in-the-loop 的核心论断成立了。** SALT-Q 7.002 对后置量化 floor 8.873,
> **赢 1.87**(3-epoch 格里只赢 0.12);QA-LoRA 赢 2.58。也就是说**只有在不过拟合的格里,
> 这一列才在说量化的事**,而它一说,结论就与 CS / MetaMath 两列一致:QAT-in-the-loop 明显
> 优于"先 fp16 微调再 PTQ"。这就是主表该用 1-epoch 格的理由。
>
> **5. QEFT 的比特预算优势几乎蒸发了。** 3 epoch 时它领先 QA-LoRA **2.13**,1 epoch 时只领先
> **0.27** —— 而它多带 0.75 个 bit。也就是说 3-epoch 那一格里 QEFT 的领先大部分来自"纯 INT2
> 臂在过拟合",一旦纯 INT2 臂不过拟合了,0.75 bit 几乎买不到东西。这个对照比单独看 QEFT 那行
> 有信息量得多。
>
> **单变量与基座复用(实现细节)**:epochs 3 → 1,四个臂一起改。T = 176(原 528),所有 lr ×√3 保持实测位移不变
> (AGENTS.md 的位移定律 —— 这样改动才是"轮数"而不是"轮数+步长")。配置是从 3-epoch 的
> 配置**机械派生**的,并且断言了 parsed diff 只碰 `num_epochs` / `output_dir` / lr 三类叶子,
> 每个 lr 恰好 ×1.7321。(这条断言当场抓到一个真 bug:朴素的数字替换把 `*_by_bits` 里的
> **比特宽度键**也缩放了,`2` → `3.46`。)
>
> **基座全部复用,不重建** —— 两格共享同一套冻结码与同一个置换:
> SALT-Q 走 `--permuted_base_dir` / `--saltq_base_dir`;QA-LoRA 的 `qalora_intb_base` 做成指向
> 3-epoch 那份的 symlink(`scripts/train.py` 从 output_dir 推导该路径且没有覆盖参数,重建虽然
> 会得到相同的码字,但那是假设而不是事实);QEFT 的 `base_dir` 在配置里没变,Stage 0 直接跳过。
>
> **预先写下的 caveat 兑现了一半**:保持位移不变意味着参数**走得还是一样远**,变的只是过数据的
> 遍数;结果是 gap 减半而非消失,而且诊断显示**残余过拟合恰好集中在 SALT-Q**(比值 1.68 vs
> 其余 1.05–1.30)。下一个杠杆就是位移本身(即**不**搬运 lr,或直接降 salient_lr / 减 group_k
> 以降容量),那是**另一个格**,不要和这个混为一谈。**未跑,待定。**
>
> 另外:`Llama-2-7B_int2_g32_k256_asym_wiki2-2bit-qeftbase-eval` 在两个独立 job 里各测了一次,
> 两次都是 **7.5383**(逐位相同)。评测管线的确定性因此是实测的,不是假设的。
>
> **HF 的 `train_loss` 不能跨 epoch 数比较**(176 步的均值被 warmup 主导,528 步的是三遍的均值)。
> 1-epoch 的 train_loss 排序 SALT-Q 2.112 > QA-LoRA 1.942 > QEFT 1.888 > QLoRA 1.666 与 3-epoch
> 的排序相反,但这**不能**推出"过拟合没了" —— 只有最终产物的 in-sample PPL 能,job 16078835
> 在用同一套协议、同样的 166 个训练窗补测。
>
> 另外,QLoRA job 顺带产出的 `-2bit-none-dequant-eval`(NF4 基座 + 导出量化器里的 INT2 RTN,
> **没有 GPTQ**)PPL = **206,908**。**它不是本列的一行**,但它是同一句话的极端版:同一类模型在
> CS 表里只会掉到接近随机猜(~36),在 PPL 上炸掉四个数量级。
...
✅ **已跑完(job 16072379,2026-09-03)。预测成立,而且比预期更强。**

| 基座 | 校准集 | tokens | CS MEAN(8)↑ | Wiki2 PPL↓ @1024 | @2048 | C4-val PPL↓ @2048 |
|---|---|---|---|---|---|---|
| fp16 参考(原始 Llama-2-7B,**协议锚点**) | — | — | — | 6.098 | **5.472** | 7.286 |
| fp16 母 ckpt(D/E 被量化前的那个 checkpoint) | — | — | — | 6.564 | 5.823 | 7.616 |
| **D** INT2 g32 | balanced 任务校准 3500 | 471k | **66.22** | 19.190 | **16.248** | **17.923** |
| **E** INT2 g32 | C4 通用 128×2048 | 262k | **30.67** | 15.229 | **12.868** | **14.788** |
| **D** INT3 g64 | balanced 任务校准 3500 | 471k | **76.05** | 7.848 | 6.901 | 8.847 |
| **E** INT3 g64 | C4 通用 128×2048 | 262k | **72.07** | 7.608 | 6.682 | 8.641 |

**INT2 的结论(要写进论文的那一行)**:E 的困惑度在**两个语料上都更好** —— WikiText-2
12.87 vs 16.25(D 差 26.3%)、C4 14.79 vs 17.92(D 差 21.2%)—— 而它的任务分**崩了 35.55 分**
(30.67 vs 66.22)。**两个指标的排序完全相反。**

比原预测更强的一点:E 不只是在**自己的**校准语料(C4)上更好 —— 那不足为奇 —— 它在
**WikiText-2 上也更好**,而 WikiText-2 是**两臂都没有校准过**的语料。所以这不是"E 过拟合了自己
的校准集",而是:**按通用语言建模的口径,E 客观上是更好的模型,它只是把任务丢了。**
GPTQ 忠实地优化了校准集提出的目标;INT2 下问错问题是致命的,而 PPL —— 量化文献最常用的那个指标
—— 完全看不见这件事。

**比特依赖(附带发现,值得写)**:同一个对照在 INT3 上几乎消失 —— E 只好 0.22 PPL(3.3%),
任务只差 3.98 分。差异随比特下降而放大约一个数量级(PPL 3.3%→26.3%,任务 3.98→35.55)。
也就是说"用通用语料校准"这个标准做法在 INT4/INT3 是无害的,**只有到 ≤2bit 才致命** ——
这正好是本文的工作区间,也解释了为什么文献里没人被这件事绊倒过。

**协议锚点 ✅**:原始 fp16 Llama-2-7B 在本协议下 seqlen 2048 测得 **5.4721**(166 窗),
即公开值 ≈5.47。`scripts/eval_ppl.py` 的 1–5 步任何改动都必须能复现这个数。
...
job:`jobs/wiki2_t8_calib_ppl.pbs`(16072379,✅ 完成)。结果落
