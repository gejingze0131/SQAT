...
**step-0 的 loss 是最有价值的早期诊断。** SALT-Q 第 0 步的模型就是 GPTQ INT3 模型本身（M2 门保证
逐位等价），而 QLoRA merged w/GPTQ 3bit g64 = 28.7 GSM8k 是个能工作的模型，所以初始 loss 应落在
**1.8–2.5 量级**（参考：QLoRA 1.0、PSQAT 1.8）。

- 初始 loss ≈ 10 → codes / scale 对不上，或 boundary gather 没注册，**立刻停**。
  （历史上 LR-QAT 那个 loss=10 正是实现 bug 的表现，不是低比特的固有现象。）
